...
FabricDFGWidget::FabricDFGWidget(QWidget *parent) : DFG::SHDFGCombinedWidget(parent), m_initialized(false) {
  GetCoreClient();
  ...
  QObject::connect(this, SIGNAL( portEditDialogInvoked(FabricUI::DFG::DFGBaseDialog *, FTL::JSONObjectEnc<>*)),
...
FabricDFGWidget *FabricDFGWidget::Instance() {
  if(!s_widget) s_widget = new FabricDFGWidget( NULL );
  return s_widget;
}

void FabricDFGWidget::Destroy() {
  if ( s_widget ) delete s_widget;
  s_widget = NULL;
...
void FabricDFGWidget::setCurrentUINodeName(const char * node) {
  ...
  if(!m_initialized) {
    FabricServices::ASTWrapper::KLASTManager *manager = ASTWrapper::KLASTManager::retainGlobalManager( &s_coreClient );
    DFG::DFGConfig config;
    config.graphConfig.useOpenGL = false;
    init( s_coreClient, manager, m_dfgHost, binding, "", exec, &m_cmdHandler, false, config );
    m_initialized = true;
  }
  else getDfgWidget()->getUIController()->setBindingExec( binding, "", exec );
}
...
void FabricDFGWidget::onPortEditDialogCreated(DFG::DFGBaseDialog * dialog) {
  ...
  if(!controller->isViewingRootGraph()) return;
