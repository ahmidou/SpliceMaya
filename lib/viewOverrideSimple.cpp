...
bool gRTRRenderEnabled = true;
FabricCore::RTVal RTRRender::mRTRRender;

bool isRTRRenderEnabled() {
  return gRTRRenderEnabled;
}

void enableRTRRender(bool enable) { 
  gRTRRenderEnabled = enable;
}
 
ViewOverrideSimple::ViewOverrideSimple(const MString &name, const MString &viewportName )
  ...
  , mCurrentOperation(-1)
{
  ...
  mOperationNames[0] = "Scene";
  mOperationNames[1] = "RTRRender";
  mOperationNames[2] = "USER_HUD";
  mOperationNames[3] = "Simple_HUD";
}
...
bool RTRRender::initRTRRender() {
  
  if(!gRTRRenderEnabled)
    return false;
 
  if(!mRTRRender.isValid()) {
    mRTRRender = FabricSplice::constructObjectRTVal("HostToRTRCallback");
    mRTRRender = mRTRRender.callMethod("HostToRTRCallback", "getOrCreateCallback", 0, 0);
  }
  else if(mRTRRender.isObject() && mRTRRender.isNullObject()) {
    mRTRRender = FabricSplice::constructObjectRTVal("HostToRTRCallback");
    mRTRRender = mRTRRender.callMethod("HostToRTRCallback", "getOrCreateCallback", 0, 0);
  }
...
MStatus RTRRender::execute(const MHWRender::MDrawContext &context) {
  ...
  try
  {
    ...
    FabricCore::RTVal viewportNameVal = FabricSplice::constructStringRTVal(mViewportName.asChar());
    FabricCore::RTVal viewport = mRTRRender.callMethod("BaseRTRViewport", "getOrAddViewport", 1, &viewportNameVal);
    ...
    mRTRRender.callMethod("", "render", 5, &parameters[0]); 
    return MStatus::kSuccess;
  }
  catch (FabricCore::Exception e)
  {
