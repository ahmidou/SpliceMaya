...
void plugToPort_compoundArray(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  ...
  if(plug.isArray()){
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, compoundVals);
  }
  else{
  }
...
void plugToPort_compound(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  if(plug.isArray()){
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, rtVal);
  }
}
...
void plugToPort_bool(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  if(plug.isArray()){
    ...
    MDataHandle handle = data.inputValue(plug);
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructBooleanRTVal(handle.asBool()));
  }
}
...
void plugToPort_integer(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  if(plug.isArray()){
    ...
    if(handle.type() == MFnData::kIntArray) {
      ...
      if(!port.isArray())
        port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructSInt32RTVal(handle.asLong()));
    }
  }
}
...
void plugToPort_scalar(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  ...
  if(plug.isArray()){
    ...
    if(port.isArray()){
      ...
      if(scalarUnit == "time")
        port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructFloat64RTVal(handle.asTime().as(MTime::kSeconds)));
      else if(scalarUnit == "angle")
        port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructFloat64RTVal(handle.asAngle().as(MAngle::kRadians)));
      else if(scalarUnit == "distance")
        port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructFloat64RTVal(handle.asDistance().as(MDistance::kMillimeters)));
      else
      {
        if(handle.numericType() == MFnNumericData::kFloat)
          port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructFloat64RTVal(handle.asFloat()));
        else
          port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructFloat64RTVal(handle.asDouble()));
      }
    }
  }
}
...
void plugToPort_string(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  ...
  if(plug.isArray()){
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, stringArrayVal);
  }
  else{
    if(port.isArray())
      return;
    MDataHandle handle = data.inputValue(plug);
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, FabricSplice::constructStringRTVal(handle.asString().asChar()));
  }
...
void plugToPort_color(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  ...
  if(plug.isArray()){
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, arrayVal);
  }
  else {
    if(port.isArray())
      return;
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, color);
  }
...
void plugToPort_vec3(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  if(plug.isArray()){
    ...
    }else
    {
      ...
      port.setRTVal_lockType(FabricSplice::LockType_Exclusive,  spliceVec3 );
    }
  }
}
...
void plugToPort_euler(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  ...
  if(plug.isArray()){
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, euler);
  }
...
void plugToPort_mat44(MPlug &plug, MDataBlock &dataBlock, FabricSplice::DGPort & port){
  if(plug.isArray()){
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, spliceMat);
  }
}
...
void plugToPort_PolygonMesh(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  ...
  try
  {
    if(plug.isArray())
    {
      portRTVal = port.getRTVal_lockType( FabricSplice::LockType_Exclusive );
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, portRTVal);
  }
  catch(FabricCore::Exception e)
  {
    mayaLogErrorFunc(e.getDesc_cstr());
...
void plugToPort_KeyframeTrack(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  if(!plug.isArray()){
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, trackVal);
  } else {
    ...
    port.setRTVal_lockType(FabricSplice::LockType_Exclusive, trackVals);
  }
}
...
void plugToPort_spliceMayaData(MPlug &plug, MDataBlock &data, FabricSplice::DGPort & port){
  try{
    ...
    if(!plug.isArray()){
      ...
      port.setRTVal_lockType(FabricSplice::LockType_Exclusive, spliceMayaData->getRTVal());
    }else{
      MArrayDataHandle arrayHandle = data.inputArrayValue(plug);
      unsigned int elements = arrayHandle.elementCount();

      FabricCore::RTVal value = port.getRTVal_lockType( FabricSplice::LockType_Exclusive );
      if(!value.isArray())
        return;
      ...
      port.setRTVal_lockType(FabricSplice::LockType_Exclusive, value);
    }
  }
  catch(FabricCore::Exception e)
  {
...
void portToPlug_compound(FabricSplice::DGPort & port, MPlug &plug, MDataBlock &data){
  ...
  FabricCore::RTVal rtVal = port.getRTVal_lockType( FabricSplice::LockType_Exclusive );
  MFnCompoundAttribute compound(plug.attribute());
...
void portToPlug_bool(FabricSplice::DGPort & port, MPlug &plug, MDataBlock &data){
  if(plug.isArray()){
    ...
    MDataHandle handle = data.outputValue(plug);
    handle.setBool(port.getRTVal_lockType( FabricSplice::LockType_Exclusive ).getBoolean());
  }
}
...
void portToPlug_string(FabricSplice::DGPort & port, MPlug &plug, MDataBlock &data){
  if(plug.isArray()){
    ...
    FabricCore::RTVal arrayVal = port.getRTVal_lockType( FabricSplice::LockType_Exclusive );
    unsigned int elements = port.getArrayCount();
    for(unsigned int i = 0; i < elements; ++i){
      MDataHandle handle = arraybuilder.addElement(i);
    ...
    MDataHandle handle = data.outputValue(plug);
    handle.setString(MString(port.getRTVal_lockType( FabricSplice::LockType_Exclusive ).getStringCString()));
  }
}
...
void portToPlug_mat44(FabricSplice::DGPort & port, MPlug &plug, MDataBlock &data){
  ...
  else
  {
    ...
    port.getRTVal_lockType( FabricSplice::LockType_Exclusive ).callMethod( "", "getTr", 1, &matrixExtArray );
...
void portToPlug_PolygonMesh(FabricSplice::DGPort & port, MPlug &plug, MDataBlock &data){
  try
  {
    if(plug.isArray())
    {
      ...
      unsigned int elements = port.getArrayCount();
      FabricCore::RTVal polygonMeshArray = port.getRTVal_lockType( FabricSplice::LockType_Exclusive );
      for(unsigned int i = 0; i < elements; ++i)
    ...
    else
    {
      ...
      portToPlug_PolygonMesh_singleMesh(handle, port.getRTVal_lockType( FabricSplice::LockType_Exclusive ));
    }
  }
  catch(FabricCore::Exception e)
  {
...
void portToPlug_Lines(FabricSplice::DGPort & port, MPlug &plug, MDataBlock &data){
  try
  {
    ...
    {
      MDataHandle handle = data.outputValue(plug.attribute());
      portToPlug_Lines_singleLines(handle, port.getRTVal_lockType( FabricSplice::LockType_Exclusive ));
    }
  }
  catch(FabricCore::Exception e)
  {
...
void portToPlug_spliceMayaData(FabricSplice::DGPort & port, MPlug &plug, MDataBlock &data){
  ...
  try{
    if(!plug.isArray()){
      ...
      spliceMayaData->setRTVal(port.getRTVal_lockType( FabricSplice::LockType_Exclusive ));
